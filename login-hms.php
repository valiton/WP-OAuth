...
$_SESSION['WPOA']['PROVIDER'] = 'HMS';
define('HTTP_UTIL', get_option('wpoa_http_util'));
define('CLIENT_ENABLED', get_option('wpoa_hms_api_enabled'));
define('CLIENT_ID', get_option('wpoa_hms_api_id'));
define('CLIENT_SECRET', get_option('wpoa_hms_api_secret'));
...
function get_oauth_token($wpoa) {
	...
	// parse the result:
	$result_obj = json_decode($result, true); // PROVIDER SPECIFIC: Facebook encodes the access token result as a querystring by default
...
function get_oauth_identity($wpoa) {
	...
	$oauth_identity['id'] = $result_obj["data"]["userKey"];
